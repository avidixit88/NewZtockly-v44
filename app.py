...
try:
    # Local helper module (preferred)
    from email_utils import send_email_alert, format_alert_email
except Exception as _e:
    # Fallback: keep app operational even if module import fails in some deployment environments.
    import smtplib
    from email.message import EmailMessage
    from typing import Dict, Any

    def send_email_alert(smtp_server: str, smtp_port: int, smtp_user: str, smtp_password: str,
    ...
    def format_alert_email(payload: Dict[str, Any]) -> str:
        # Minimal fallback formatter; detailed formatting lives in email_utils.py
        lines = []
        lines.append(f"Symbol: {payload.get('Symbol')}")
        lines.append(f"Signal: {payload.get('Signal')}")
        if payload.get("Stage"):
            lines.append(f"Stage: {payload.get('Stage')}")
        if payload.get("SwingStage"):
            lines.append(f"SwingStage: {payload.get('SwingStage')}")
        for k in ["Entry", "Stop", "TP1", "TP2", "PullbackBand", "EntryZone", "TrendLockScore",
                  "RetracePct", "PullbackQuality", "ConfluenceCount", "EntryTriggerReason"]:
            if k in payload and payload.get(k) not in (None, "", "N/A"):
